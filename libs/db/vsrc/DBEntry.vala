...
class shotodol.db.DBEntryCell : Replicable {
...
public class shotodol.db.DBEntryFactory : Replicable {
	// factory setup
...
public class shotodol.db.DBEntry : Searchable {
	Bundler bndlr;
	bool closed;
	...
	public int addTxt(int index, xtring val) throws BundlerError {
		if(closed) {
			throw new db_entry.entry_error.entry_closed("entry closed\n");
		}
	...
	public int addETxt(int index, extring*val) throws BundlerError {
		if(closed) {
			throw new db_entry.entry_error.entry_closed("entry closed\n");
		}
		bndlr.writeEXtring((aroop_uword8)index, val);
